...
描述飽和岩石電阻率(𝑅𝑜)與孔隙率(𝜙)之關係


#### Lithology Coefficient 岩性係數(a)

用於糾正孔隙結構和晶粒尺寸的變化，與電流的路徑長度有關，值介於 0.5 到 1.5，在不同的儲層中可能有所不同

有些人認為岩性係數與曲折度有關
  - 曲折度𝝉（Tortuosity factor） = 𝑪 / 𝑳
  - 𝐶：曲線長度
  - 𝐿：兩端之間距離

#### 孔隙水電阻率(Rw)

其值隨著水中離子含量減少而增加，也受到水溫的影響，溫度越高，其值越低。現地測量數值一般介於 1 到 10 


#### Porosity 孔隙率(ϕ)

為孔隙體積和岩體總體積的比率，值介於 0 到 1
  
  - 𝝓 = 𝑽𝒘𝒂𝒕𝒆𝒓 + 𝑽𝒂𝒊+ 𝑽𝒂𝒊𝒓 / 𝑽𝒕𝒐𝒕𝒂𝒍
...
#### Cementation exponent 岩體的膠結指數(m)

用來計算岩石孔隙結構的一個指數，由於裂縫和孔洞的存在，導致了儲層的非均質性，進而造成電參數改變。受孔隙度、滲透率和泥質含量等多種因素的控制，通常認為不受溫度控制。
值介於 1.7 到 3，一般以 2 為正常值



#### Formation factor 地層因子(F)

Archie 提出地層因子(𝑭)以描述岩體之孔隙結構與膠結程度
...
岩體中水含量的多少，一般用比值來表示，大小可以從0（完全乾燥）到與該材料的孔隙度相同（即含水量達到飽和狀態）
...
#### Saturation exponent 飽和度指數(n)

流體與岩體關係的指數，與岩石的潤濕程度有關，介於 1.8 到 4 ，一般以 2 為正常值
...
- 地電阻剖面影像法( SSI 或 ERT )
  - 使⽤固定電極間隔⽣成⽔平剖⾯（電阻率的橫向變化）或同時⽣成電阻率的橫向和垂直變化

### CH7.4.1 Vertical electrical sounding (VES)


#### 雙偶極陣列

－ 增加偶極間的間隔，⽽不是通過延⾧電流電極來增加穿透深度

－ 測量位置取電極陣列的中點

－ 電阻(δV/I)的測量是在最短的電極間隔下進行的，然後間距再逐漸增⼤

#### （修正）Wenner 陣列

－ 隨著電極間距的增加，四個電極都必須移動到新位置（Wenner 陣列）

－ 使⽤特殊的多芯電纜

－ 利用轉換器來完成 Wenner陣列垂直電探測深（不用手動更改電極）

#### Schlumberger陣列

-  使用較少的空間( 相較於 Wenner 陣列 )
